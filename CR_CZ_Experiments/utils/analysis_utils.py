...
def local_fidelity_optimization(channel: Choi,
    ...
    def fidelity_objective(params):
        ...
        return 1 - qi.process_fidelity(channel, opt_oper, require_cp=False)

    def to_favg(val):
        return (4 * val + 1) / 5

    raw_fid = qi.process_fidelity(channel, target_op, require_cp=False)
...
class ExperimentRunner:
    ...
    def run(self, qobjs: List[PulseQobj], file_name: str, bool_from_ids=False) -> Union[List[Result], Result]:
        if not isinstance(qobjs, list):
            qobjs = [qobjs]

        file_path = '%s/%s.%s' % (self._path, file_name, ExperimentRunner._ext)
        file_path_jobids = '%s/%s.%s' % (self._path, "jobids"+file_name, ExperimentRunner._ext)
        
        if bool_from_ids == True:
            with open(file_path_jobids, 'r') as fp:
                jobid_dict = json.load(fp)
                jobids = [x for x in jobid_dict.keys()]
                jobs = []
                for jobid in jobids:
                    job = self._backend.retrieve_job(jobid)
                    jobs.append(job)
        else:
            if not self._cached:
                jobs = []
                ...
                for qobj in qobjs:
                    ...
                    jobids.append(jobid)
                print(jobids)
                jobid_dict = dict.fromkeys(jobids)

                with open(file_path_jobids, 'w') as fp:
                    json.dump(jobid_dict, fp, cls=IQXJsonEncoder, default=_more_serialization)
            ...
            if len(job_results) != len(jobs):
                print(len(job_results), 'results of', len(jobs))
    ...
    def _save(results: List[Result], file_path: str):
        ...
        with open(file_path, 'w') as fp:
            json.dump(dict_results, fp, cls=IQXJsonEncoder, default=_more_serialization)
...
def _more_serialization(o):
    """A helper function to convert datetime object."""
